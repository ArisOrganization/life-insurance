		...
		const app = {
			...
			general_js: {
				...
				will_be_in_viewport: function will_be_in_viewport(elem, space) {
					...
					return (
						(bounding.top - space) <= (window.innerHeight || document.documentElement.clientHeight) &&
						bounding.left >= 0 &&
						bounding.bottom >= 0 &&
						bounding.right <= (window.innerWidth || document.documentElement.clientWidth)
					);
				},
				wait_for_final_event: function wait_for_final_event(callback, ms, uniqueId) {
			...
			form: {
					...
					$(".form-next").on("click", function(e){
						...
						if(_this.custom_validate()){ 
							...
							if($(".active-step").hasClass("step-2")){
								_this.form_data.cover_value = $(".active-step select option:selected").val()
								_this.form_data.cover_length = $("#cover-length").val()

							}else if($(".active-step").hasClass("step-3")){

								_this.form_data.name_1 = $("#name-1").val()
								_this.form_data.dob_1 = $("#date-day-1 option:selected").val() + "-" +$("#date-month-1 option:selected").val()  + "-" + $("#date-year-1 option:selected").val() 
 
								console.log("dob " + _this.form_data.dob_1 )
								if(_this.form_data.cover_group != "me"){
									_this.form_data.name_2 = $("#name-2").val()
									_this.form_data.dob_2 = $("#date-day-2 option:selected").val() + "-" +$("#date-month-2 option:selected").val()  + "-" + $("#date-year-2 option:selected").val() 
								} 
				...
				custom_validate: function custom_validate(){
					...
					if($(".active-step").hasClass("step-2")){
						if($(".active-step select option:selected").val() == "empty" || $("#cover-length").val().length < 1)
							return false
						...
						if(_this.form_data.terms_agree){
							if(!$("#telephone").hasClass("valid") || !$("#email").hasClass("valid") ){
				...
				submit_form_result: function submit_form_result(){
					...
					let _this = this;
					console.log("FORM OBJECT: " + JSON.stringify(_this.form_data, 2, null))
					let payload = { 
						method: 'submit_form', 
				...
				init: function init(){
					...
					$(window).bind("load", function() {  
						setTimeout(function(){  
								...
								widget.src = "https://widget.reviews.co.uk/rich-snippet-reviews-widgets/dist.js" 
							...
							if($("form").length){ 
								...
								data_8.src = "https://webservices.data-8.co.uk/javascript/loader.ashx?key="+ api_key +"&load=InternationalTelephoneValidation,EmailValidation" 
									...
									$("form").validate(
										{
											rules: {
												telephone: {
													...
													d8val_inttelephone_opt: [{name : "RequiredCountry", value: "GB"}]
												}
											}
										}
									); 
								}
								data_8_validate.src = "https://webservices.data-8.co.uk/javascript/jqueryvalidation_min.js" 
			...
			db_req:{
				send:  async function send(payload, debug = false) {
					return await fetch("/api/endpoint.php", {  // PRODUCTION ENDPOINT
						method: 'post',
						...
						.then(data => {
							if(debug) console.log('DEBUG response: ' + payload.method + ' : ' + JSON.stringify(data, null, 2).replace(/\\"/g, '').replace(/<br\s*[\/]?>/gi, ' \n ') );
							return data
						})
